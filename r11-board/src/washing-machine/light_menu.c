...
HWND HOSTING;
static newGapX = 0;
static unsigned int volumelength = 0;
static int lightlength = 0;
...
static int MainWinProc(HWND hWnd, int message, WPARAM wParam, LPARAM lParam) {
	...
	case MSG_CLOSE:
		EnableWindow(GetWifiWnd(), TRUE);
                ReleaseCapture();
		unregister_res();
		mwd->volumelen = volumelength;
		mwd->lightlen = lightlength;
	...
	case MSG_PAINT:
		...
		case 3:
			...
			RoundRect(hdc, 40, 150, volumelength, 170, 10, 10);
		...
		case 4:
			...
			FillBoxWithBitmap(hdc, 10, 150, 360, 20, &volume_bar_n);
			SetBrushColor(hdc, PIXEL_yellow);
			RoundRect(hdc, 40, 150, lightlength, 170, 10, 10);
			//FillBoxWithBitmap(hdc, 30, 150, length - 30, 20, &progress_p);
			break;
		case 5:
			SelectFont(hdc, Font_45);
	...
	case MSG_LBUTTONDOWN:
		...
		case 3:
			...
			ScreenToClient(hWnd, &lengthX, &lengthY);
			if (lengthX < 350 && lengthX > 40 && lengthY > 145 && lengthY < 175) {
				volumelength = lengthX;
				lightlength = lengthX;
	...
	case MSG_LBUTTONUP:
		...
		case 3:
			...
			if (LOWORD(lParam) < 200 || HIWORD(lParam) > 380 || LOWORD(lParam) > 600 || HIWORD(lParam) < 100) {
				if (msgclose == 1) {
					break;
				} else if (msgclose == 0) {
		...
		case 11:
			...
			break;
		case 13:{
                        ScreenToClient(hWnd, &oldX, &oldY);
			printf("oldX[%d] oldY[%d]\n", oldX, oldY);
			if(oldX > 0 && oldX < 400 && oldY > 0 && oldY < 280)
			if(msgclose == 1){
				break;
			}else if(msgclose == 0){
				ReleaseCapture();
	...
	case MSG_MOUSEMOVE: {
		...
		case 3:
			...
			if (isSlidVolume) {
				ScreenToClient(hWnd, &oldX, &oldY);
				if (oldX < 350 && oldX > 40 && oldY > 145 && oldY < 175) {
					volumelength = oldX;
					lightlength = oldX;
					InvalidateRect(hWnd, NULL, TRUE);
				} else {
...
int light_menu(HWND hParent, int id, int volumelen, int lightlen) {
	if (lightmenuWnd != 0) {
		return -1;
	}
	HOSTING = hParent;
	...
	lightmenuWnd = CreateMainWindow(&CreateInfo);
	if (lightmenuWnd == HWND_INVALID)
		return -1;
	if(ID != 2 )
	{
		SetCapture(lightmenuWnd);
	}
	EnableWindow(GetWifiWnd(), FALSE);
	ShowWindow(lightmenuWnd, SW_SHOWNORMAL);
	msgclose = 0;

	while (GetMessage(&Msg, lightmenuWnd)) {
		TranslateMessage(&Msg);
		DispatchMessage(&Msg);
	}
	MainWindowThreadCleanup(lightmenuWnd);
	lightmenuWnd = 0;
	return 0;
}
