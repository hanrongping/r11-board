...
extern int RestrictedButtonViewProc(HWND hwnd, int message, WPARAM wParam, LPARAM lParam);
...
static int HeadBarViewProc(HWND hwnd, int message, WPARAM wParam, LPARAM lParam) {
	...
	case MSG_CREATE: {
		...
		old_buttonview_proc = GetWindowCallbackProc(btn_home);
		SetWindowCallbackProc(btn_home, RestrictedButtonViewProc);
...
HWND HeadBarViewInit(HWND hParent, HWND myBottomBarHwnd,
	...
	if (strcmp(headBarData->winDes, WIN_DES_MAIN) == 0) {
		mHwnd = CreateWindowEx(HEAD_BAR_VIEW, "", WS_CHILD | WS_VISIBLE, WS_EX_TRANSPARENT, 0, 0, 0, 800, 100,
					hParent, (DWORD) headBarData);
	}
	else{
		mHwnd = CreateWindowEx(HEAD_BAR_VIEW, "", WS_CHILD, WS_EX_TRANSPARENT, 0, 0, 0, 800, 100, 
					hParent, (DWORD) headBarData);
	}
