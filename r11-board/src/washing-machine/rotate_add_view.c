...
static int RotateAddViewProc(HWND hwnd, int message, WPARAM wParam,
	...
	case MSG_PAINT: {
		...
		if (selectStatus == SELECT_STATUS_NO) {
			...
			SelectFont(hdc, gongnengfontBig);
			FillBoxWithBitmap(hdc, 158, 33, 0, 0, &rotateAddView->bmpAnim[1]);
		}
